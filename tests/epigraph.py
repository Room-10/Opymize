...
x_in = np.zeros((nregions, nfuns, 3))
x_out = x_in.copy()
x = x_in.copy()
...
for i in range(10):
    ...
    x_in[0,0,:] = -5.0 + 10*np.random.rand(3)
    x_in[0,0,-1] = -b[0,0] + v[0].dot(x_in[0,0,:2])

    # x_in[0,1] : point on the boundary
    x_in[0,1,:] = -5.0 + 10*np.random.rand(3)
    vals = [-bk + vk.dot(x_in[0,1,:2]) for bk,vk in zip(b[1,1:],v[1:])]
    active_k = np.argmax(vals)
    x_in[0,1,-1] = vals[active_k]

    # x : point inside half space (mapped to itself)
    x = x_in.copy()
    x[0,:,-1] += 4*np.random.rand(2)
    op(x, x_out)
    assert np.linalg.norm(x_out - x) < tol
    ...
    x[0,0,:2] += shift[0]*v[0]
    x[0,0,-1] -= shift[0]
    x[0,1,:2] += shift[1]*v[1+active_k]
    x[0,1,-1] -= shift[1]
    op(x, x_out)
    assert np.linalg.norm(x_out - x_in) < tol
